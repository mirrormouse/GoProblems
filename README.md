...
### Q.00 Hello World!
以下と同じになるようにプログラムQ00.goを書き換え、「使い方」の説明に従ってテストせよ。「PASS」と表示されれば成功である。以降の問題も同様にテストすること。
...
### Q.01 平均の計算１
2つの整数x,yを入力として受け取り、この平均を出力として返す関数Averageを実装せよ。ただし小数点以下は絶対値が小さくなるように切り捨て、整数として出力すること。
...
### Q.02 if文の使用
3つの整数a,b,cを入力として受け取り、この最大値を出力として返す関数Maxを実装せよ。
...
### Q.03 for文の使用
整数xを受け取りxの2乗を出力として返す関数Squareを実装せよ。また正の整数nを受け取り、1からnまでの値の2乗の和を返す
...
### Q.04 switch文の使用
正の整数nを受け取り、4で割った余りを英語表記した文字列を出力として返す関数Mod4を実装せよ。表記は小文字でも大文字でも構わない。
...
### Q.05 文字列結合演算子「+」
文字列sと文字列tを受け取り、s,tを結合したうえで末尾に"!"を付けた文字列を出力として返す関数StringConcatを実装せよ。
...
### Q.06 累乗関数
整数xと自然数aを受け取り、xのa乗を返す関数mypowを実装せよ。なお、ここで自然数は0を含むものとする。
...
### Q.12 配列の総和
長さ5の整数の配列sを受け取り、sに格納されている値の総和を返す関数Arraysumを実装せよ。
...
### Q.13 スライスの２乗
整数のスライスsを受け取り、sに格納されている各値を２乗した配列を返す関数Arraysquareを実装せよ。
...
以下の操作をiを1からNまで変化させて順に行ったとき、得られるスライスtを求める関数Evensplitを実装せよ。
操作:<br>
sのi番目の要素$s_{i}$が偶数ならば$\frac{s_{i}}{2}$を2つ、奇数であれば$s_{i}$を1つ、tの末尾に追加する。
...
このとき、盤面上に存在する自分の駒の数から相手の駒の数を引いた値を出力する関数Boardgameを実装せよ。
| 入力例 | 出力例 |
| ---- | ---- |
...
### Q.17 出現回数数え上げ
文字列のスライスSとTを受け取る。Tの各要素について、その要素がSの中に何度登場するかを求め、整数のスライスとして出力する関数Selectstringを実装せよ。

| 入力例 | 出力例 |
| ---- | ---- |
| {apple,orange,apple,apple,orange},{apple,orange} | {3,2} |
| {one,one,two,three,four,two,three,two},{two,four} | {3,1} |
| {Monday,Monday,Tuesday,Friday,Saturday,Thursday,Monday,Monday},{Monday,Friday,Sunday} | {4,1,0} |


### Q.18 最小公倍数と最大公約数
正の整数n,mを受け取り、nとmの最大公約数を求める関数GCDを実装し、関数GCDをもとに最小公倍数を求める関数LCMを実装せよ。テストは関数LCMに対してのみ実行される。
...
### Q.19 ナベアツ問題
正の整数nを受け取り、1からnまでの値i ($1\leq i\leq n$) について、iが3の倍数または10進数表記した際に3が現れる数である場合は数字iの末尾に「!」をつけて、そうでなければ数字iを空白区切りで順に標準出力に表示する関数Nabeatsuを実装せよ。
...
### Q.21 数値の抽出
文字列Sを入力として受け取り、Sの中に含まれている整数をすべて抽出し、出現順に並べて整数のスライスとして出力する関数StringNumberを実装せよ。
ただし、整数が連続して現れる場合は1つの数値とみなし、0で始まる数値については不要な0を取り除くこと。

| 入力例 | 出力例 |
| ---- | ---- |
| abc1def23ghi | {1,23} |
| 60kg170.01cm | {60,170,1} |
| 000.063/023.980 | {0,63,23,980} |
